# Week 03 - Tinh Nang Va Chuc Nang Da Them
...
- Xu ly event `ENROLLMENT_CONFIRMED` tu enrollment-service de tang `enrolled_count`.
...
- Co outbox worker doc event va goi course-service de cap nhat `enrolled_count`.
...
- `docker-compose.yml` chay Postgres, 3 backend service va GraphQL gateway.
...
- Kiem tra `enrolledCount` tang sau khi outbox worker xu ly event.
