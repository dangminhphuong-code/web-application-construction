...
async function connectWithRetry() {
  for (;;) {
    try {
      ...
      console.error("[course-event-publisher] connect failed:", error.message);
      await sleep(CONNECT_RETRY_DELAY_MS);
    }
  }
}

export async function connectCourseEventPublisher() {
  if (channel) {
    return channel;
  }
  ...
  connecting = (async () => {
    ...
    connection.on("error", (error) => {
      console.error("[course-event-publisher] connection error:", error.message);
    });

    connection.on("close", () => {
      console.error("[course-event-publisher] connection closed");
      connection = null;
      channel = null;
      connecting = null;
    });
    ...
    console.log(`[course-event-publisher] connected. exchange=${EXCHANGE}`);
    return channel;
  })();
...
export async function publishCourseEvent({ routingKey, event }) {
  const ch = await connectCourseEventPublisher();
...
export async function closeCourseEventPublisher() {
  if (channel) {
    await channel.close();
  }
